...
def create_table_transaction(dir_in, dir_out):
    ...
    #df_accHold = pd.read_csv(dir_out + "accountHolders.csv")
    #map_accId = pd.read_csv(dir_in + "accountIdMap.csv")
    #map_accId = map_accId.set_index("accountID").accountIdentifier.to_dict() 
    #df_mapUnit = pd.read_csv(dir_in + "unitTypeCodes.csv")
    #df_mapRegistry = pd.read_csv(dir_in + "registryCodes.csv")
    ...
    # impose codes for lookup tables
    df["unitTypeCode"] = df.unitType.map(map_unitType_inv)
    df["transactionTypeMain"] = df["transactionType"].map(lambda x: int(x.split("-")[0]))
    df["transactionTypeSupplementary"] = df["transactionType"].map(lambda x: int(x.split("-")[1]))
