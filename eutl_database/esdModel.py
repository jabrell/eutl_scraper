...
class EsdAccount(Base):
    __tablename__ = "esd_account"

    id = Column(Integer(), primary_key=True)
    accountID = Column(Integer)
    accountHolder = Column(String(50), ForeignKey("esd_account_holder.id"))
    yearValid = Column(Integer)
    name = Column(String(250))
    tradingSystem = Column(String(250))


class EsdAccountHolder(Base):
    __tablename__ = "esd_account_holder"

    id = Column(String(50), primary_key=True)
    name = Column(Integer)
    memberState = Column(String(250))
    yearValid = Column(Integer)
    name = Column(String(250))
    tradingSystem = Column(String(250))
...
class Account(Base):
    ...
    accountHolder_id = Column(Integer(), ForeignKey(
        "account_holder.id"), index=True)
    accountType_id = Column(String(10), ForeignKey(
        "account_type_code.id"), index=True)
    ...
    installation_id = Column(
        String(100), ForeignKey("installation.id"), index=True)
    created_on = Column(DateTime(), default=datetime.now)
    updated_on = Column(DateTime(), default=datetime.now,
                        onupdate=datetime.now)
    # orbis_id = Column(String(100))
...
class AccountHolder(Base):
    ...
    accounts = relationship(
        "Account", back_populates="accountHolder", lazy=True)
...
class Installation(Base):
    ...
    activityType = relationship(
        "ActivityType", backref="installations_in_country", foreign_keys=[activity_id], lazy=True)
    registry = relationship("CountryCode", backref="installations_in_registry", foreign_keys=[
                            registry_id], lazy=True)
    country = relationship("CountryCode", backref="installations_in_country", foreign_keys=[
                           country_id], lazy=True)
...
class Compliance(Base):
    ...
    installation_id = Column(String(100), ForeignKey(
        "installation.id"), primary_key=True)
    ...
    installation = relationship(
        "Installation", backref="compliance", lazy=True)
...
class Surrender(Base):
    ...
    # relations
    installation = relationship(
        "Installation", backref="surrendering", lazy=True)
    unitType = relationship("UnitType", lazy=True)
