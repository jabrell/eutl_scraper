...
def export_mappings(dir_out):
    ...
    for fn, data in maps.items():
        pd.DataFrame(data.items(), columns=["code", "description"]
                     ).to_csv(dir_out + fn, index=False)
...
map_activity = {20: "Combustion of fuels",
...
map_activity_data_to_code = {"%d-%s" %
                             (k, v): k for k, v in map_activity.items()}
map_activity_data_to_name = {"%d-%s" %
                             (k, v): v for k, v in map_activity.items()}


map_complianceCode = {"A": u"The number of allowances and ERUs/CERs surrendered by 30 April is greater than or equal to verified emissions",
                      "B": u"The number of allowances and ERUs/CERs surrendered by 30 April is lower than verified emissions",
                      "C": u"Verified emissions were not entered until 30 April",
                      "D": u"Verified emissions were corrected by competent authority after 30 April of year X. The competent authority of the Member State decided that the installation is not in compliance for year X-1",
                      "E": u"Verified emissions were corrected by competent authority after 30 April of year X. The competent authority of the Member State decided that the installation is in compliance for year X-1",
                      "X": u"Entering verified emissions and/or surrendering was impossible until 30 April due to the allowance surrender process and/or verified emissions update process being suspended for the Member State's registry",
                      "-": u"No Compliance Obligations"}


map_trans_type = {1: "Issuance - Initial creation of a unit",
...
map_trans_supp_type = {0: "No Supplementary Type",
