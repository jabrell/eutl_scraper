...
def parse_ec_transactions(
    fn, fn_file="transactions_EUTL_PUBLIC_NOTESD_20220502.csv", dir_out=None
):
    ...
    cols = [
        ...
        "TRACK",
        "AMOUNT",
        "TRANSFERRING_REGISTRY_NAME",
        "ACQUIRING_REGISTRY_NAME",
        "TRANSFERRING_ACCOUNT_NAME",
        "ACQUIRING_ACCOUNT_NAME",
        "TRANSFERRING_ACCOUNT_IDENTIFIER",
        "ACQUIRING_ACCOUNT_IDENTIFIER",
    ]

    df = df[cols].copy()
    # get transaction types
    df["transactionTypeMain_id"] = df["TRANSACTION_TYPE"].map(lambda x: x.split("-")[0])
    df["transactionTypeSupplementary_id"] = df["TRANSACTION_TYPE"].map(
        lambda x: x.split("-")[1]
    )
    ...
    def create_account_id(row, prefix):
        if pd.notnull(row[prefix.upper() + "_ACCOUNT_IDENTIFIER"]):
            return (
                row[prefix + "RegistryCode"]
                + "_%d" % row[prefix.upper() + "_ACCOUNT_IDENTIFIER"]
            )

    # convert registries to codes
    for pf in ["acquiring", "transferring"]:
        # convert registries to codes
        df[pf + "RegistryCode"] = df[pf.upper() + "_REGISTRY_NAME"].map(
            lambda x: map_registryCode_inv.get(x.strip())
        )
        # Create an account identifier unique in the transaction database
        df[pf + "AccountIdentifierDB"] = df.apply(
            lambda x: create_account_id(x, pf), axis=1
        )
...
def link_accounts(dir_in, df=None, tries=3, wait=5, use_existing=False):
    ...
    :param use_existing: <bool> if True
        Existing account mapping is used and only missing account identifiers
        are linked
    """
    fn_mapping = dir_in + "account_mapping.csv"
    dir_out = dir_in + "transactionBlocks.csv"
    ...
    acc_id_db = list(
        set(
            list(df.transferringAccountIdentifierDB.unique())
    ...
    if use_existing:
        df_map_existing = pd.read_csv(fn_mapping)
        acc_id_db = [
            i
            for i in acc_id_db
    ...
    def is_success(x):
        return (
            b"An error occurred during execution of the request. Please try again later or redefine the request."
            not in x
        )
    ...
    for i, accIdentifierDB in enumerate(acc_id_db):
        if i % 100 == 0:
            print(
                "Link accountIdentifier %s (%d/%d)"
                % (accIdentifierDB, i, len(acc_id_db))
            )
        ...
        content = download_file(
            base_url, parms=query, tries=tries, wait=wait, test_success=is_success
        )
    ...
    df_map = pd.DataFrame(
        [[k, v] for k, v in res.items()], columns=["accountIdentifierDB", "accountID"]
    )

    if use_existing:
        df_map = pd.concat([df_map_existing, df_map], axis=0)
        assert (
            df_map.duplicated().sum() == 0
        ), "Duplicated account identifiers in mapping to transaction data"
        assert (
            df_map.accountIdentifierDB.duplicated().sum() == 0
        ), "Duplicated account identifiers in mapping to transaction data"
        assert (
