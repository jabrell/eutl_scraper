...
class TransactionSpider(scrapy.Spider):
    ...
    def __init__(self, start_date="", end_date="", fn_accountIdentifiers=None, **kwargs):
        ...
        self.start_urls = [f"{self.url_transaction_overview}?{urllib.parse.urlencode(self.params_transaction_search)}"]
    ...
    def parse(self, response):
        ...
        for row in rows[2:]:
            ...
            for i, c in enumerate(cols):
                ...
                transactionDate = row.css("td:nth-child(3)>span.classictext::text").get().strip()
                transactionType = row.css("td:nth-child(2)>span.classictext::text").get().strip()
                transferringRegistry = row.css("td:nth-child(5)>span.classictext::text").get().strip()
                acquiringRegistry = row.css("td:nth-child(10)>span.classictext::text").get().strip()
    ...
    def parse_transaction_blocks(self, response):
        ...
        for row in rows[2:]:
            ...
            if link is not None:
                l.add_css("transferringAccountName", "td:nth-child(5)>a.resultlink>span.resultlink::text")
                l.add_value("transferringAccountURL", link.strip())
            else:
                l.add_css("transferringAccountName", "td:nth-child(5)>span.classictext::text") 
            ...
            if link is not None:            
                l.add_css("acquiringAccountName", "td:nth-child(7)>a.resultlink>span.resultlink::text")
                l.add_value("acquiringAccountURL", link.strip())
                #l.add_css("acquiringAccountURL", "td:nth-child(7)>a.resultlink::attr(href)")
            else:
                l.add_css("acquiringAccountName", "td:nth-child(7)>span.classictext::text")
                ...
                for i in range(1, max_pages + 1):
                    ...
                    yield response.follow(url, callback=self.parse_transaction_blocks,
                                          meta={"transactionID": response.meta["transactionID"],
                                                ...
                                                "transferringRegistry": response.meta["transferringRegistry"]})
